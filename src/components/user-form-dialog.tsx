...
import { FormState, FormUpdateProps } from "@/types/form"
...
const UserUpdateDialog = React.forwardRef<HTMLFormElement, UserUpdateDialogProps>(
	...
	const [formData, setFormData] = useState<FormUpdateProps<UserUpdateForm>>({
		originalData: {
			type: user.type,
			fullName: user.name,
		...
		hasChanged: (data, originalData) => {
			const currentDataJson = JSON.stringify(data)
			const originalDataJson = JSON.stringify(originalData)

			return currentDataJson !== originalDataJson
		}
	})
	...
	function onCloseDialog(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
		if (formData.hasChanged(form.getValues(), formData.originalData)) {
			e.preventDefault()
			setIsAlertOpen(true)
			return
		}
